...
class HashTable:
    ...
    def put(self, key, value):
        ...
        # check if item already in the hash table
            # traverse the linkedlist to update
        while cur:
            if cur.key ==key:
                cur.value = value
                return False
        ...
        # if the number of item is 70% of cap-> double the table size
    ...
    def delete(self, key):
        ...
        prev = None # for reconnecting the node
        cur = self.buckets[index]
        ...
        while cur:
            if cur.key ==key:
                # prev is None if head-> then one element in linkedlist
                if prev is None:
                    # just update the bucket element itself
                ...
                # if the number of item is 30% of cap-> half the table size
                # but not smaller than 97
    ...
    def rehash(self, old):
        ...
        # traverse to get all existing items into new hash
        ...
        for bucket in old:
            # we need to traverse each bucket's linkedlist
            cur = bucket
            while cur:
